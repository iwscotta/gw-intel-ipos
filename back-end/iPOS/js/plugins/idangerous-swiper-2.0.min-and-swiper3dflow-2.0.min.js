    ...
    if (window.getComputedStyle || (window.getComputedStyle = function(a) {
        ...
        e.wrapper = f, e._extendSwiperSlide = function(a) {
            return a.append = function() {
                ...
                return b === void 0 ? a.innerHTML : (a.innerHTML = toStaticHTML( b, a))
            }, a.index = function() {
            ...
            return f.innerHTML = toStaticHTML( a || "", f.className = c, e._extendSwiperSlide(f))
        ...
        e.startAutoplay = function() {
            ...
            if (0 != e.slides.length) {
                ...
                for (f.innerHTML = toStaticHTML ( c + f.innerHTML + a), e.loopCreated = !0, e.calcSlides(), d = 0; e.slides.length > d; d++)(e.loopedSlides > d || d >= e.slides.length - e.loopedSlides) && e.slides[d].setData("looped", !0);
...
Swiper.prototype.plugins.tdFlow = function(e, t) {
    function f() {
        ...
        for (var t = 0; t < n.length; t++) {
            e.setTransition(n[t], 0)
        }
        if (o) {
            r = e.h.getWidth(e.wrapper);
            i = r / n.length;
            for (var t = 0; t < n.length; t++) {
                n[t].swiperSlideOffset = n[t].offsetLeft
            }
        } else {
            r = e.h.getHeight(e.wrapper);
            i = r / n.length;
            for (var t = 0; t < n.length; t++) {
                n[t].swiperSlideOffset = n[t].offsetTop
            }
        }
    }
    ...
    function l(n) {
        ...
        var r = o ? -n.x + e.width / 2 : -n.y + e.height / 2;
        var u = o ? t.rotate : -t.rotate;
        var a = t.depth;
        for (var f = 0; f < e.slides.length; f++) {
            var l = e.slides[f].swiperSlideOffset;
            var c = (r - l - i / 2) / i * t.modifier;
            var h = o ? u * c : 0;
            var p = o ? 0 : u * c;
            var d = -a * Math.abs(c);
            var v = o ? 0 : t.stretch * c;
            var m = o ? t.stretch * c : 0;
            if (Math.abs(m) < .001) m = 0;
            if (Math.abs(v) < .001) v = 0;
            if (Math.abs(d) < .001) d = 0;
            if (Math.abs(h) < .001) h = 0;
            if (Math.abs(p) < .001) p = 0;
            var g = "translate3d(" + m + "px," + v + "px," + d + "px)  rotateX(" + p + "deg) rotateY(" + h + "deg)";
            e.setTransform(e.slides[f], g);
            e.slides[f].style.zIndex = -Math.abs(Math.round(c)) + 1;
            if (t.shadows) {
                var y = o ? e.slides[f].querySelector(".swiper-slide-shadow-left") : e.slides[f].querySelector(".swiper-slide-shadow-top");
                var b = o ? e.slides[f].querySelector(".swiper-slide-shadow-right") : e.slides[f].querySelector(".swiper-slide-shadow-bottom");
                b.style.opacity = -c > 0 ? -c : 0;
                y.style.opacity = c > 0 ? c : 0
            }
        }
        if (e.ie10) {
            var w = e.wrapper.style;
            w.perspectiveOrigin = r + "px 50%"
        }
    }
    if (!e.support.transforms3d) return;
    var n, r, i, s;
    var o = e.params.mode == "horizontal";
    if (!t) return;
    var u = {
        rotate: 50,
        stretch: 0,
    ...
    var c = {
        onFirstInit: function(r) {
            n = e.slides;
            if (t.shadows) {
                var i = document.createElement("div");
                var s = document.createElement("div");
                i.className = o ? "swiper-slide-shadow-left" : "swiper-slide-shadow-top";
                s.className = o ? "swiper-slide-shadow-right" : "swiper-slide-shadow-bottom";
                for (var u = 0; u < n.length; u++) {
                    n[u].appendChild(i.cloneNode());
                    n[u].appendChild(s.cloneNode())
                }
            }
            f();
            l({
        ...
        onSetWrapperTransition: function(n) {
            for (var r = 0; r < e.slides.length; r++) {
                e.setTransition(e.slides[r], n.duration);
                if (o && t.shadows) {
                    e.setTransition(e.slides[r].querySelector(".swiper-slide-shadow-left"), n.duration);
                    e.setTransition(e.slides[r].querySelector(".swiper-slide-shadow-right"), n.duration)
                } else if (t.shadows) {
                    e.setTransition(e.slides[r].querySelector(".swiper-slide-shadow-top"), n.duration);
                    e.setTransition(e.slides[r].querySelector(".swiper-slide-shadow-bottom"), n.duration)
                }
            }
        }
    };
